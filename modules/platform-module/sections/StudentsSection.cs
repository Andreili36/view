...
{
    public class StudentsSection : SectionBase
    {
        public override string Title => "Студенты";
        public override bool IsVisible => RoleId == 1;

        public StudentsSection(PlatformMain host) : base(host) { }
        ...
        private List<Student> LoadData()
        {
            return Db.SqlQuery(
                @"SELECT id, last_name, first_name, middle_name, birth_date,
                     gender, phone, email, registration_date
        ...
        {
            var s = new Student();
            if (!TryEditDialog("Новый студент", s)) return;

            try
            {
                Db.SqlExecute(
                    @"INSERT INTO student (last_name, first_name, middle_name, birth_date,
                                       gender, phone, email, registration_date)
                  VALUES (@ln, @fn, @mn, @bd, @g, @ph, @em, @rd)",
                    new()
                    {
                        ["@ln"] = s.LastName,
                        ["@fn"] = s.FirstName,
                        ...
                        ["@ph"] = s.Phone,
                        ["@em"] = s.Email,
                        ["@rd"] = s.RegistrationDate
                    });
                Host.ShowMessage("Студент добавлен.");
                Host.RefreshCurrent();
            }
        ...
        private void OnEdit(FrameworkElement root)
        {
            ...
            if (table.SelectedItem is not Student s) { Host.ShowError("Выберите студента."); return; }
            if (!TryEditDialog("Редактирование студента", s)) return;

            try
            {
                Db.SqlExecute(
                    @"UPDATE student SET last_name=@ln, first_name=@fn, middle_name=@mn,
                         birth_date=@bd, gender=@g, phone=@ph, email=@em
                  WHERE id=@id",
                    new()
                    {
                        ["@ln"] = s.LastName,
        ...
        private void OnDelete(FrameworkElement root)
        {
            var table = (DataGrid)((Grid)root).Children[1];
            if (table.SelectedItem is not Student s) { Host.ShowError("Выберите студента."); return; }
            if (!Host.Confirm($"Удалить студента «{s.FullName}»?")) return;

            try
            {
                Db.SqlExecute("DELETE FROM student WHERE id=@id", new() { ["@id"] = s.Id });
        ...
        private bool TryEditDialog(string title, Student s)
        {
